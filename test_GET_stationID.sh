...
# GetClient requests for a valid station ID
curl -X GET "http://127.0.0.1:4567/weather/IDS01" > output.txt

# Compare
file1="./test_GET_stationID.txt"
file2="output.txt"

# Compare the two files using the `cmp` command
if cmp -s "$file1" "$file2"; then
    echo -e "\e[32mPassed test: GET by station ID \e[0m"
else
    echo -e "\e[31mFailed test: GET by station ID \e[0m"
fi
